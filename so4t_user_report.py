...
def main():
    ...
    # Filter to top N most recently active users if specified
    if args.top_active:
        users = get_top_active_users(users, args.top_active)
        print(f"Filtered to {len(users)} most recently active users.")
...
def get_args():
    ...
    parser.add_argument('--top-active',
                        type=int,
                        default=None,
                        help='Return only the N most recently active users. Default: all users.')
...
def add_new_user_fields(users):
    ...
    for user in users:
        ...
        # Process badge counts
        if 'badge_counts' in user and isinstance(user['badge_counts'], dict):
            user['bronze_badges'] = user['badge_counts'].get('bronze', 0)
            user['silver_badges'] = user['badge_counts'].get('silver', 0)
            user['gold_badges'] = user['badge_counts'].get('gold', 0)
        else:
            user['bronze_badges'] = 0
            user['silver_badges'] = 0
            user['gold_badges'] = 0

        # Keep the original badge_counts for backward compatibility
        user['badge_counts'] = user['bronze_badges'] + user['silver_badges'] + user['gold_badges']
...
def create_user_report(users, start_date, end_date):
    ...
    for user in sorted_users:
        user_metric = {
            ...
            'Last Access Date': format_timestamp(user['last_access_date']),
            ...
            'Bronze Badges': user['bronze_badges'],
            'Silver Badges': user['silver_badges'],
            'Gold Badges': user['gold_badges'],
...
def initialize_deleted_user(user_id, display_name):
    ...
    user = {
        ...
        'reputation_history': [],
        'net_reputation': 0,
        'badge_counts': 0,  # Deleted users have no badges
        'bronze_badges': 0,
        'silver_badges': 0,
        'gold_badges': 0,
...
def get_top_active_users(users, limit):
    """
    Returns the N most recently active users based on their last_access_date.
    
    Args:
        users: List of user dictionaries
        limit: Number of most recently active users to return
    
    Returns:
        List of the N most recently active users
    """
    # Sort users by last_access_date in descending order (most recent first)
    sorted_users = sorted(users, key=lambda user: user.get('last_access_date', 0), reverse=True)
...
def format_timestamp(timestamp):
    ...
    Returns:
        Formatted date string in YYYY-MM-DD HH:MM:SS format, or 'Unknown' if invalid
    """
    try:
